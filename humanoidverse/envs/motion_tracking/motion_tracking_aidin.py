...
from humanoidverse.utils.motion_lib.motion_lib_robot import MotionLibRobot, LocoMujocoMotionLibRobot
...
class LeggedRobotAidinMotionTracking(LeggedRobotBase):
    ...
    def _init_motion_lib(self):
        ...
        self._motion_lib = LocoMujocoMotionLibRobot(self.config.robot.motion, num_envs=self.num_envs, device=self.device, simulator=self.simulator)
    ...
    def _init_motion_extend(self):
        if "extend_config" in self.config.robot.motion:
            ...
            self.ref_body_pos_extend = torch.zeros(self.num_envs, self.num_bodies, 3, dtype=torch.float, device=self.device, requires_grad=False)
            self.dif_global_body_pos = torch.zeros(self.num_envs, self.num_bodies, 3, dtype=torch.float, device=self.device, requires_grad=False)
            self.marker_coords = torch.zeros(self.num_envs, 
                                         self.num_bodies, 
                                         3, 
                                         dtype=torch.float, 
    ...
    def _pre_compute_observations_callback(self):
        ...
        self.dif_global_body_pos = ref_body_pos_extend - self.simulator._rigid_body_pos
        # import ipdb; ipdb.set_trace()
        ## diff compute - kinematic rotation
        self.dif_global_body_rot = quat_mul(ref_body_rot_extend, quat_conjugate(self.simulator._rigid_body_rot, w_last=True), w_last=True)
        
        ## diff compute - kinematic velocity
        self.dif_global_body_vel = ref_body_vel_extend - self.simulator._rigid_body_vel
        ## diff compute - kinematic angular velocity
        
        self.dif_global_body_ang_vel = ref_body_ang_vel_extend - self.simulator._rigid_body_ang_vel
        ...
        dif_global_body_pos_for_obs_compute = ref_body_pos_extend.view(env_batch_size, -1, 3) - self.simulator._rigid_body_pos.view(env_batch_size, -1, 3)
        ...
        # heading_inv_rot_vr = heading_inv_rot.repeat(1,1)
